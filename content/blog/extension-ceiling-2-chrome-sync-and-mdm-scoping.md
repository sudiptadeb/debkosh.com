...
- `SyncDisabled` kills sync for the profiles it reaches. Total and blunt, and users notice their bookmarks stopped following them.
- `SyncTypesListDisabled` turns off individual types. Most teams disable passwords and autofill, keep bookmarks, and call it scoped.
- `BrowserSignin` can block browser sign-in entirely, or force it so the profile is always managed.
- `RestrictSigninToPattern` is the precise one: a pattern over which accounts may sign in, usually anchored to the corporate domain.
